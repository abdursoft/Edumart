                        ...
                        <td>
                            <div class="flex items-center gap-1">
                                <a href="{{ route('page', $page->slug) }}" target="_blank"
                                    class="py-1 px-2 rounded-[15px] shadow-md hover:bg-gray-600 text-slate-600 hover:bg-gray-800 hover:text-white transition">{!! icons('view') !!}</a>
                                <a href="{{ route('admin.page.create_page.edit', $page->id) }}"
                                    class="py-1 px-2 rounded-[15px] shadow-md hover:bg-green-600 text-green-600 hover:text-white transition">{!! icons('edit') !!}</a>
                                <a href="{{ route('admin.page.create_page.delete', $page->id) }}"
                                    class="py-1 px-2 text-red-600 rounded-[15px] shadow-md hover:bg-red-600 hover:text-white transition"
                                    onclick="return confirm('Are you sure you want to delete this page?')">{!! icons('delete') !!}</a>
                            </div>
                        </td>
                    </tr>
                @endforeach
...
@push('scripts')
    {{ loadDataTable('example', 'Page list') }}
@endpush
