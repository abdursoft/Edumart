        ...
        <div class="owl-carousel owl-theme owl-loaded owl-drag h-[60vh]">
            @if (!empty($slider))
                @foreach ($slider->contents as $content)
                    <div class="relative w-full h-full" style="background:url('{{ Storage::url($content->media_url) }}') center;background-size:cover;background-repeat:no-repeat;background-position:center;">
