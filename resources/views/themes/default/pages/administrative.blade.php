...
@section('content')
    ...
    <div class="w-full {{count($persons) > 0 ? 'grid' : ''}} grid-cols-1 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-2 my-2 py-10 px-2 md:px-5">
        @forelse($persons as $person)
            @if(!empty($person->profile))
                <div class="w-ful h-[280px] rounded-md overflow-hidden shadow-md">
                    <img src="{{asset($person->profile?->avatar)}}" alt="{{$person->first_name ?? ''}}" class="w-full h-[180px]">
                    <h2 class="text-xl">{{$person->profile?->first_name. ' '.$person->profile?->last_name}} <small class="text-sm">({{ucfirst($person->role)}})</small></h2>
